...
export async function POST(request: Request) {
  ...
  // Find eligible pieces: belong to this client, no body yet, currently
  // PLANNED (we deliberately skip WRITING to avoid double-processing a
  // concurrent batch).
  ...
  if (candidates.length === 0) {
    return NextResponse.json({
      started: 0,
      message: "Nothing to generate — every piece already has a draft or is in progress.",
    });
  }

  const pieceIds = candidates.map((p) => p.id);
...
export async function GET(request: Request) {
  ...
  // Three counts the UI cares about:
  //   - writing:  currently being generated by some worker (status=WRITING + no body)
  //   - pending:  still queued for generation (no body, not WRITING)
  //   - drafted:  body is populated (DRAFT_REVIEW or later)
  // When `writing` is 0 the batch is finished; `pending > 0` means some
  // pieces failed and were reset, so the user can re-batch them.
  const [writing, pending, drafted] = await Promise.all([
    prisma.contentPiece.count({
      where: { contentPlan: { clientId }, status: "WRITING", body: null },
    }),
    prisma.contentPiece.count({
      where: {
        contentPlan: { clientId },
    ...
    prisma.contentPiece.count({
      where: { contentPlan: { clientId }, body: { not: null } },
    }),
  ]);
