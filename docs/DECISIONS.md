...
26. **Phase 10 — Published Content Library, Internal Linking, Distribution Assets** (D41). Three coupled features shipped together so they reinforce each other on the first publish. (a) **Schema:** new columns on `ContentPiece` (`slug`, `metaDescription`, `socialPosts` JSON), new `InternalLink` model with PENDING/RESOLVED/BROKEN states and a `plannedSlug` field, migration `20260512_phase10_content_library` (idempotent — `ADD COLUMN IF NOT EXISTS`, DO-block FK guards). (b) **Slug generation:** `generateSlug(title, keyword)` helper produces kebab-case from the title at draft time so future drafts can reference this piece's slug *before* it publishes. Operator can override at publish time via the manual edit modal. (c) **Draft-time enrichment:** `generateContentBodyInternal()` now (1) pulls the client's published library and injects it as a manifest in the body-generation prompt so Claude can write real internal links `[anchor](slug)` to topically-relevant prior pieces; (2) runs a second Claude call after the body is finalized to produce `metaDescription` (150–160 chars) and `socialPosts` for Twitter/X (240 chars), LinkedIn (600–800), Facebook (400–600), and Instagram (caption + hashtags); (3) calls `syncInternalLinks()` to mirror the `[anchor](slug)` placeholders Claude wrote into the InternalLink table. (d) **Link resolution at publish time:** the WordPress `POST /publish` endpoint walks the body, looks each non-URL link target up against the client's published-piece slugs, and rewrites resolved links to real URLs while *stripping* (keeping anchor text only) unresolved links — broken links are worse than no link. After publish, every PENDING InternalLink pointing to the *just-published* piece's slug flips to RESOLVED. (e) **PATCH endpoint** accepts `slug`, `metaDescription`, `socialPosts`, `publishedUrl`, `publishedAt` and re-syncs InternalLinks whenever the body changes. Manual "Mark Published" now auto-generates a slug if missing and resolves inbound pending links the same way the WP path does. (f) **Agency edit modal** got a tab strip: a "Body" tab (existing) and a new "Distribution (SEO + Social)" tab with slug input, meta description (with char counter — turns red over 160), and four per-platform social-post textareas each with a "Copy" button. (g) **Client-facing "Your Content" tab** at `/client/[token]/library`: a `Library` icon nav item alongside Dashboard / Content Review / Reports. Renders every published piece newest-first with type badge, publish date, target keyword, meta description, and a "View live" button. A "Show share-ready social copy" expander opens per-platform copy with "Copy with link" buttons that append the published URL to the caption so the client can paste straight into any social tool. Filterable/searchable by title/keyword and by type. **Deferred (out of this batch):** auto-edit of source-WP-posts when a *future* piece publishes so the resolved-after-publish links go live on the existing posts (needs Yoast/Rank Math detection + a robust WP REST update flow + idempotency tracking — added to Phase 10 backlog), and the nightly broken-link sweep (needs scheduler infrastructure). For now resolved-after-publish links flip in the DB but the live posts retain the anchor-text-only version — the operator can re-publish the source piece manually to push the updated body.

25. **Agency-side feedback loop: surfacing client edits + manual editor + working Publish tab** (D40). After D39 shipped the client-side draft review, several gaps in the agency flow surfaced: (a) sidebar dots on the client portal showed every piece green on first load because the dot logic was reading `p.approval?.outcome` (which still held the "approved" outcome from the plan-review cycle), not the client's current draft decision; (b) the "X drafts sent for client review" badge in the agency Drafts tab was reading "currently in CLIENT_REVIEW," so the count decremented as the client decided pieces; (c) when the client clicked Request Edits, their `notes` and any inline annotations weren't visible anywhere in the agency UI; (d) the agency couldn't edit a draft body manually — only Regenerate (full rewrite) was available; (e) the Publishing tab filtered on the unused `READY_TO_PUBLISH` status, so APPROVED pieces never appeared and the publish button was unreachable. Fixes: (1) Client sidebar dot logic in `DraftReviewMode` now sources truth from `decisions[p.id]` + the piece's current `status` field (APPROVED/REJECTED/PUBLISHED), explicitly ignoring the stale plan-review `approval.outcome`. Same change applied to the per-group "X / N" counter and the header "X of Y reviewed" counter. (2) Agency "drafts sent" label rewritten to count pieces that ever left for client review (`CLIENT_REVIEW + APPROVED + REJECTED + PUBLISHED`), with an "X awaiting decision" suffix that does decrement so the operator still gets a sense of progress. (3) `/api/clients/[clientId]` GET now eagerly includes `annotations` on every piece. The agency Drafts tab piece card grew two new sections: a yellow "Client requested edits" block showing the client's overall note, and a "Inline comments from client" panel listing each unresolved annotation with the highlighted quote and the comment. (4) New `PATCH /api/content/pieces/[id]` accepting `{ body, title, status, resolveAnnotationIds }`. A new modal opens from an "Edit" button on each piece card with a body textarea (markdown), a sidebar showing all client feedback in context, and a "Save Edit" action that transitions the piece back to `DRAFT_REVIEW`, bumps `revisionCount`, and marks any unresolved annotations as `resolved`. (5) Publishing tab filter changed from `READY_TO_PUBLISH` to `APPROVED || READY_TO_PUBLISH` (with `body` non-null). Each ready piece now shows two buttons: a primary green "Push to WordPress" that calls the existing `/api/content/pieces/[id]/publish` endpoint (defaults to `status=draft` so an editor can review on-site), and a secondary "Mark Published Manually" for clients without WP integration. Also added a "Publish" button directly on the Drafts tab card for APPROVED pieces, so the operator doesn't have to switch tabs.
